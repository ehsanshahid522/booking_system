...
const TODAY_SCHEDULE = [
  { id: '1', customerName: 'Oliver Smith', service: 'VIP Full Grooming Package', amount: 65, status: 'completed', time: '10:00 AM' },
  { id: '2', customerName: 'Jack Williams', service: 'Classic Cut & Styling', amount: 25, status: 'completed', time: '11:30 AM' },
  { id: '3', customerName: 'Harry Brown', service: 'Hot Towel Shave', amount: 22, status: 'completed', time: '02:00 PM' },
];
  ...
  return (
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      <View style={styles.header}>
        <Text style={styles.title}>Salon Earnings (£ GBP)</Text>
      ...
      <View style={styles.totalCard}>
        ...
        <Text style={styles.totalAmount}>£{(period === 'week' ? total : total * 4.3).toLocaleString('en-GB', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}</Text>
        <View style={styles.totalStats}>
          <View style={styles.totalStat}>
            <Text style={styles.totalStatValue}>{period === 'week' ? '42' : '180+'}</Text>
          ...
          <View style={styles.totalStatDivider} />
          <View style={styles.totalStat}>
            <Text style={styles.totalStatValue}>£{todayEarned.toLocaleString()}</Text>
      ...
      <View style={styles.sectionHeader}>
        <Text style={styles.sectionTitle}>Today&apos;s Completed Transactions</Text>
      </View>
      {TODAY_SCHEDULE.map(apt => (
        <View key={apt.id} style={styles.payRow}>
          ...
          <View style={styles.payRight}>
            <Text style={styles.payAmount}>+ £{apt.amount.toLocaleString()}</Text>
...
const styles = StyleSheet.create({
  ...
  barAmount: { color: Colors.textMuted, fontSize: 10, marginBottom: 2, position: 'absolute', top: 0 },
