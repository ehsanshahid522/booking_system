...
export default function ChatScreen() {
  ...
  function openChat(conv: typeof CHAT_CONVERSATIONS[0]) {
    setActiveChat(conv);
    setView('chat');
  }
  ...
  function sendMessage(text: string) {
    if (!text.trim()) return;
    setMessages(prev => [...prev, { id: `m${Date.now()}`, senderId: 'c1', text: text.trim(), time: 'Just now', isRead: false }]);
    setInput('');
  }
  ...
  return (
    <View style={styles.container}>
      ...
      {CHAT_CONVERSATIONS.map(conv => (
        <TouchableOpacity key={conv.id} style={styles.convRow} onPress={() => openChat(conv)} activeOpacity={0.8}>
      ...
      <View style={styles.empty}>
        <Text style={styles.emptyNote}>💬 All your booking chats appear here</Text>
      </View>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  emptyNote: { color: Colors.textMuted, fontSize: 13 },
  // Chat Room
